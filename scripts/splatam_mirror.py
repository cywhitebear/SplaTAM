...
def get_mirror_mask(frame_idx, work_dir):
    mask_image_path = os.path.join(work_dir, "mask", f"{frame_idx}.png")
    mask_image = cv2.imread(mask_image_path, cv2.IMREAD_GRAYSCALE)
    if mask_image is None:
        raise FileNotFoundError(f"Mask image {mask_image_path} not found.")

    # Resize the mask to match the depth image size (720, 960)
    mask_resized = cv2.resize(mask_image, (960, 720), interpolation=cv2.INTER_NEAREST)
    # mask_flattened = mask_image.reshape(-1)
    mask_binary = mask_resized == 0
    # get_mirror_mask(time_idx, config['workdir'])
    return mask_binary
...
def rgbd_slam(config: dict):
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        # Mask out mirror depth values
        mirror_mask = get_mirror_mask(time_idx, config['workdir'])
        # Convert the mask to the same device as depth and convert to float
        mirror_mask_tensor = torch.tensor(mirror_mask, device=depth.device, dtype=torch.float32)
        # Apply the mask to the depth image
        depth[0] = depth[0] * mirror_mask_tensor
        depth[0][mirror_mask_tensor == 0] = float('nan')
